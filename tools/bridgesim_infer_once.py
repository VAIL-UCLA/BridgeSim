...
def world_to_ego_local(ego_xyz: np.ndarray, ego_rotmats: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """
    Convert world-frame history to ego-local frame at t0.
    """
    t0_xyz = ego_xyz[:, :, -1, :]
    t0_R = ego_rotmats[:, :, -1, :, :]
...
def load_frames_as_expected(image_npy: str, target_n: int = 16) -> torch.Tensor:
    ...
    We accept (H, W, 3) or (N, H, W, 3) and normalize to (target_n, 3, H, W).

    - If fewer than target_n: left-pad with earliest
    - If more than target_n: keep most recent target_n
    """
...
def load_ego_history(ego_xyz_npy: str, ego_rot_npy: str, required_T: int = 16):
    """
    Load ego history arrays and normalize to required_T.
    Supports ego_rot as:
      - heading/yaw: (1,1,T) or (1,1,T,1)
      - rotmats:     (1,1,T,3,3)
    Converts world -> ego-local at t0.
    """
...
def main():
    ...
    frames = load_frames_as_expected(args.image_npy, target_n=16)
    ...
    ego_history_xyz, ego_history_rot = load_ego_history(args.ego_xyz_npy, args.ego_rot_npy, required_T=16)
