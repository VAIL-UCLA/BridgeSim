...
from metadrive.component.static_object.traffic_object import TrafficCone, TrafficBarrier, TrafficStopSign
...
class ScenarioTrafficManager(BaseManager):
        ...
        for scenario_id, track in self.current_traffic_data.items():
            ...
            if track["type"] == MetaDriveType.VEHICLE:
                ...
                self.spawn_pedestrian(scenario_id, track)
                                   ...
                                   MetaDriveType.TRAFFIC_STOP_SIGN, MetaDriveType.STOP_SIGN]:
                # Map types to classes
                if track["type"] == MetaDriveType.TRAFFIC_BARRIER:
                    cls = TrafficBarrier
                elif track["type"] in [MetaDriveType.TRAFFIC_STOP_SIGN, MetaDriveType.STOP_SIGN]:
                    cls = TrafficStopSign
                else:  # TRAFFIC_CONE
                    cls = TrafficCone
                self.spawn_static_object(cls, scenario_id, track)
            elif track["type"] == MetaDriveType.TRAFFIC_OBJECT:
                # Skip TRAFFIC_OBJECT for now
                logger.info(f"[DEBUG after_reset] Skipping TRAFFIC_OBJECT (type: {track['type']})")
                continue
            else:
                # DEBUG: Show exact comparison values
                logger.warning(f"[DEBUG after_reset] Do not support type: '{track['type']}' (repr: {repr(track['type'])})")
                logger.warning(f"[DEBUG after_reset] STOP_SIGN constant: '{MetaDriveType.STOP_SIGN}' (repr: {repr(MetaDriveType.STOP_SIGN)})")
                logger.warning(f"[DEBUG after_reset] Check result: {track['type'] in [MetaDriveType.STOP_SIGN, MetaDriveType.TRAFFIC_OBJECT]}")
            ...
            for scenario_id, track in self.current_traffic_data.items():
                ...
                if scenario_id not in self._scenario_id_to_obj_id:
                    if track["type"] == MetaDriveType.VEHICLE:
                        ...
                        # Skip TRAFFIC_OBJECT for now
                        logger.info(f"[DEBUG after_step] Skipping TRAFFIC_OBJECT (type: {track['type']})")
                        continue
                    else:
                        # DEBUG: Show exact comparison values
                        logger.info(f"[DEBUG] Do not support type: '{track['type']}' (repr: {repr(track['type'])})")
                        logger.info(f"[DEBUG] STOP_SIGN constant: '{MetaDriveType.STOP_SIGN}' (repr: {repr(MetaDriveType.STOP_SIGN)})")
                        logger.info(f"[DEBUG] TRAFFIC_OBJECT constant: '{MetaDriveType.TRAFFIC_OBJECT}' (repr: {repr(MetaDriveType.TRAFFIC_OBJECT)})")
    ...
    def is_static_object(self, obj_id):
        return isinstance(self.spawned_objects[obj_id], TrafficBarrier) \
            ...
            or isinstance(self.spawned_objects[obj_id], TrafficStopSign)
