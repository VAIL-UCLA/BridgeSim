...
class TrafficStopSign(TrafficObject):
    """A stop sign"""

    HEIGHT = 2.5
    MASS = 5
    RADIUS = 0.3
    CLASS_NAME = MetaDriveType.TRAFFIC_STOP_SIGN

    def __init__(self, position, heading_theta, lane=None, static: bool = True, random_seed=None, name=None):
        super(TrafficStopSign, self).__init__(position, heading_theta, lane, random_seed, name)
        ...
        self.set_static(static)
        if self.render:
            if TrafficStopSign.MODEL is None:
                model = self.loader.loadModel(AssetLoader.file_path("models", "stop_sign", "stop_sign.glb"))
                model.setScale(0.5)
                model.setPos(0, 0, -self.HEIGHT / 2)
                TrafficStopSign.MODEL = model
            TrafficStopSign.MODEL.instanceTo(self.origin)
    ...
    @property
    def top_down_color(self):
        return 255, 0, 0  # Red color for stop sign
