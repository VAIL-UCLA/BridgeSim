...
def calculate_fov_from_intrinsics(K: np.ndarray, image_width: int, image_height: int) -> Tuple[float, float]:
    """
...
def convert_camera_params_to_simple_format(camera_params: dict, 
                                           image_width: int = 1920, 
                                           image_height: int = 1120,
                                           to_metadrive: bool = False) -> dict:
    """
    ...
    Coordinate System Notes:
        RAP/nuScenes: X=Forward, Y=Left, Z=Up, Yaw around Z (0° = +X)
        MetaDrive: Y=Forward, X=Right, Z=Up, Heading around Z (0° = +Y)
    """
    simple_format = {}
    ...
    for cam_name, params in camera_params.items():
        ...
        if to_metadrive:
            ...
            # Orientation: Add 90° to account for coordinate system rotation
            # RAP yaw 0° points to +X (forward in RAP), MetaDrive heading 0° points to +Y (forward in MD)
            # Since we're also flipping the Y-axis (left->right), we need +90° not -90°
            # MetaDrive_heading = RAP_yaw + 90°
            yaw_md = yaw + 90.0
...
def reconstruct_lane_boundaries(polyline: np.ndarray, widths: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    Reconstructs 3D left and right lane boundaries from a centerline and width array.
    """
    if len(polyline) < 2:
    ...
    directions = np.vstack([directions, directions[-1:]]) # Handle last point
    ...
    # Add Z coordinate back (using 0 for simplicity)
...
class ScenarioRenderer:
    ...
    def observe(self, scenario):
        # *** MODIFIED LINE ***
        # Use the 3D global position of the ego vehicle, not [0,0,0]
        ...
        for cam_id, cam_model in self.camera_models.items():
            ...
            for feat in scenario['map_features'].values():
                ...
                if 'LANE' in ftype:      
                    if 'polygon' in feat:
                        ...
                        # Distance check (using the centerline)
                        pts_dist = np.linalg.norm(polyline_3d[:, :2] - lidar_pos[np.newaxis, :2], axis=1)
                        if np.min(pts_dist) > self.depth_max:
                            continue
                        
                        left_boundary, right_boundary = reconstruct_lane_boundaries(polyline_3d, widths)
                        
                        # Draw the boundaries as polylines
                        draw_polyline_depth(canvas, left_boundary, T_w2c, K, COLOR_TABLE['lanelines'], radius=2, depth_max=self.depth_max)
                        draw_polyline_depth(canvas, right_boundary, T_w2c, K, COLOR_TABLE['lanelines'], radius=2, depth_max=self.depth_max)

                elif 'CROSSWALK' in ftype or 'SPEED_BUMP' in ftype:
                    poly2d = feat['polygon'].astype(np.float32)
                    # Use only x, y for polygon
            ...
            # Check if 'anns' exists before processing
            if "anns" in scenario:
                anns = scenario["anns"]
