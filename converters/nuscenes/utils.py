...
from converters.nuscenes.type import ALL_TYPE, HUMAN_TYPE, BICYCLE_TYPE, VEHICLE_TYPE
...
def parse_frame(frame, nuscenes: NuScenes):
    ...
    for cam in CAM_NAMES:
        if cam in frame["data"]:
            sample_data_token = frame["data"][cam]
            sd_record = nuscenes.get("sample_data", sample_data_token)
            images[cam] = os.path.basename(sd_record["filename"])
    ret["images"] = images

    return ret


def interpolate_heading(heading_data, old_valid, new_valid, num_to_interpolate=6):
    ...
    for k, valid in enumerate(old_valid[:-1]):
        if abs(valid) > 1e-1 and abs(old_valid[k + 1]) > 1e-1:
            ...
            interpolate_heading = np.linspace(heading_data[k], heading_data[k] + diff, num_to_interpolate + 1)
...
def _interpolate_one_dim(data, old_valid, new_valid, num_to_interpolate=6):
...
def interpolate(origin_y, valid, new_valid, num_to_interpolate=6):
    if len(origin_y.shape) == 1:
        ret = _interpolate_one_dim(origin_y, valid, new_valid, num_to_interpolate=num_to_interpolate)
    elif len(origin_y.shape) == 2:
        ret = []
        for dim in range(origin_y.shape[-1]):
            new_y = _interpolate_one_dim(origin_y[..., dim], valid, new_valid, num_to_interpolate=num_to_interpolate)
...
def get_tracks_from_frames(nuscenes: NuScenes, scene_info, frames, num_to_interpolate=6):
    episode_len = len(frames)

    all_objs = set()
    for frame in frames:
        keys = [k for k in frame.keys() if k != "images"]
        all_objs.update(keys)
    ...
    for frame_idx in range(episode_len):
        ...
        if "images" in frame_data:
            frame_info_list.append(frame_data["images"])
        else:
            frame_info_list.append({})

        for id, state in frame_data.items():
    ...
    for id, track, in tracks.items():
        ...
        interpolate_tracks[id]["state"]["position"] = interpolate(
            track["state"]["position"], track["state"]["valid"], new_valid, num_to_interpolate
        )

        # Velocity
        interpolate_tracks[id]["state"]["velocity"] = interpolate(
            track["state"]["velocity"], track["state"]["valid"], new_valid, num_to_interpolate
        )
        vel = interpolate_tracks[id]["state"]["position"][1:] - interpolate_tracks[id]["state"]["position"][:-1]
        dt = 0.5 / num_to_interpolate
        interpolate_tracks[id]["state"]["velocity"][:-1] = vel[..., :2] / dt
        ...
        # Heading
        new_heading = interpolate_heading(track["state"]["heading"], track["state"]["valid"], new_valid, num_to_interpolate)
        ...
        for k, v in track["state"].items():
            if k in ["valid", "heading", "position", "velocity"]:
                ...
                interpolate_tracks[id]["state"][k] = interpolate(v, track["state"]["valid"], new_valid, num_to_interpolate)

    return interpolate_tracks, frame_info_list
...
def get_map_features(scene_info, nuscenes: NuScenes, map_center, radius=500, points_distance=1, only_lane=False):
    ...
    layer_names = [
        ...
        'ped_crossing', 'walkway', 'lane_connector',
    ...
    if not only_lane:
        ...
        for id in map_objs["drivable_area"]:
            ...
            for polygon_token in seg_info["polygon_tokens"]:
                polygons.append(map_api.extract_polygon(polygon_token))
        ...
        for id in map_objs["ped_crossing"]:
            ...
            ret[id] = {SD.TYPE: MetaDriveType.CROSSWALK, SD.POLYGON: boundary_polygon}

        for id in map_objs["walkway"]:
            info = map_api.get("walkway", id)
            boundary = map_api.extract_polygon(info["polygon_token"]).exterior.xy
            boundary_polygon = np.asarray([[boundary[0][i], boundary[1][i]] for i in range(len(boundary[0]))])
            ret[id] = {SD.TYPE: MetaDriveType.BOUNDARY_SIDEWALK, SD.POLYGON: boundary_polygon}
...
def convert_nuscenes_scenario(
    ...
    if prediction:
        # Prediction split logic (unchanged)
    ...
    # 2 Hz input -> 10 Hz output (5x interpolation)
    num_steps = 5
    scenario_log_interval = 0.5 / num_steps
    ...
    result[SD.LENGTH] = (len(frames) - 1) * num_steps + 1
    result[SD.METADATA] = {}
    ...
    result[SD.METADATA][SD.TIMESTEP] = np.arange(0., (len(frames) - 1) * 0.5 + scenario_log_interval, scenario_log_interval)

    tracks, frame_info = get_tracks_from_frames(nuscenes, scene_info, frames, num_steps)

    result[SD.TRACKS] = tracks
    result[SD.METADATA]["frame_info"] = frame_info
...
def get_nuscenes_prediction_split(dataroot, version, past, future, num_workers=2):
    nusc = NuScenes(version="v1.0-mini" if "mini" in version else "v1.0-trainval", dataroot=dataroot)
