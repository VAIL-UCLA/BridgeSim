...
class UniADTrack(MVXTwoStageDetector):
    ...
    def _forward_single_frame_inference(
        ...
        # NOTE: You can replace BEVFormer with other BEV encoder and provide bev_embed here
        if use_bev_calibrator:
            bev_embed, bev_pos = self.get_bevs(img, img_metas, prev_bev=prev_bev)
            # Apply BEV calibration for domain adaptation (MetaDrive -> Bench2Drive)
            if hasattr(self, 'bev_calibrator') and self.bev_calibrator is not None:
                # Calibrate BEV: transforms from MetaDrive domain to Bench2Drive domain
                # calibrate_with_reshape returns (B, C, H, W) format
                bev_embed_calibrated = self.bev_calibrator.calibrate_with_reshape(bev_embed)

                # Normalize to (H*W, B, C) format expected by downstream heads
                bev_h = getattr(self.pts_bbox_head, "bev_h", 200)
                bev_w = getattr(self.pts_bbox_head, "bev_w", 200)
                B, C, H, W = bev_embed_calibrated.shape

                # Reshape: (B, C, H, W) -> (B, H*W, C) -> (H*W, B, C)
                if H != bev_h or W != bev_w:
                    # Interpolate if size mismatch
                    import torch.nn.functional as F
                    bev_embed_calibrated = F.interpolate(
                        bev_embed_calibrated, size=(bev_h, bev_w),
                        mode='bilinear', align_corners=False
                    )
                bev_embed = bev_embed_calibrated.flatten(2).permute(2, 0, 1)  # (H*W, B, C)
        else:
            bev_embed, bev_pos = self.get_bevs(img, img_metas, prev_bev=prev_bev)
