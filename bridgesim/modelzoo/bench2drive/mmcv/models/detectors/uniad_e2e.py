...
class UniAD(UniADTrack):
    ...
    def forward(self, inputs, return_loss=True, rescale=False, use_bev_calibrator=False):
        ...
        if return_loss:
            ...
            outputs = self.forward_test(**inputs, rescale=rescale, use_bev_calibrator=use_bev_calibrator)
            return outputs
    ...
    def forward_test(self,
        ...
        result_track = self.simple_test_track(img, l2g_t, l2g_r_mat, img_metas, timestamp, use_bev_calibrator=use_bev_calibrator)
        self._last_track_output_test = result_track[0].copy()

        # Upsample bev for tiny model        
        result_track[0] = self.upsample_bev_if_tiny(result_track[0])
        
        bev_embed = result_track[0]["bev_embed"]
        self._last_bev_embed_test = bev_embed.detach() # Store BEV embedding
        ...
        if self.with_seg_head:
            ...
            self._last_seg_output_test = result_seg[0].copy() # Store segmentation output
        ...
        if self.with_motion_head:
            ...
            self._last_motion_output_test = outs_motion.copy() # Store motion output
        ...
        if self.with_occ_head:
            ...
            self._last_occ_output_test = outs_occ.copy() # Store occupancy output
        ...
        if self.with_planning_head:
            ...
            self._last_planning_output_test = result_planning.copy() # Store planning output
            result[0]['planning'] = dict(
                planning_gt=planning_gt,
                result_planning=result_planning,
            )

        # Store BEV embed before popping if requested
        if hasattr(self, 'save_bev_for_agent') and self.save_bev_for_agent:
            # Use detach() instead of clone() to avoid memory issues
            self._last_bev_embed = bev_embed.detach()
