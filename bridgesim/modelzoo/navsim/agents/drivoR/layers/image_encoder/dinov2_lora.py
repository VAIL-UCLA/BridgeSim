...
from bridgesim.modelzoo.navsim.agents.drivoR.utils import pylogger
...
class ImgEncoder(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        # Handle model_weights - if None or empty, let timm download automatically
        model_weights = getattr(config, 'model_weights', None)
        if model_weights:
            pretrained_cfg_overlay = {"file": model_weights}
        else:
            pretrained_cfg_overlay = None

        in_chans = config.in_chans if "in_chans" in config else 3
        no_embed_class = getattr(config, 'no_embed_class', True)  # Default True to match checkpoint
        # Don't load pretrained weights if no_embed_class is True (incompatible pos_embed)
        # Our checkpoint will provide the weights anyway
        load_pretrained = not no_embed_class
        self.model = timm.create_model(model_name,
                                       pretrained=load_pretrained,
                                       pretrained_cfg_overlay=pretrained_cfg_overlay if load_pretrained else None,
