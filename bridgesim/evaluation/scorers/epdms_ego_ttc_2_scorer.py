...
Scoring formula:
  (DAC x DDC x TLC) x (100xCOL + 0xEP + 2xLK + 2xHC + 2xEC) / 106
...
class EPDMSEgoTTC2Scorer(BaseTrajectoryScorer):
    """
    ...
    Formula: (DAC x DDC x TLC) x (100xCOL + 0xEP + 2xLK + 2xHC + 2xEC) / 106
    """
    ...
    def initialize(self, scenario_data: dict, env):
        ...
        if sdc_track['state']['valid'][0]:
            ...
            print(f"[EPDMS Ego TTC 2] Calibrated World->Sim Offset: {self.world_to_sim_offset}")
        else:
            self.world_to_sim_offset = np.array([0.0, 0.0])
            print("[EPDMS Ego TTC 2] Could not calibrate coordinates (Frame 0 invalid).")

        self.prev_frame_idx = None
        self._initialized = True
        print(f"[EPDMS Ego TTC 2] Initialized with {len(self.all_lanes)} lanes (NC/TTC via linear interpolation)")
    ...
    def select_best(self, model_output: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        if not self._initialized:
            raise RuntimeError("EPDMSEgoTTC2Scorer not initialized.")
        ...
        for i in range(N):
            ...
            # (DAC x DDC x TLC) x (100xCOL + 5xEP + 2xLK + 2xHC + 2xEC) / 111
        ...
        print(f"[EPDMS Ego TTC 2] Frame {frame_idx}: best_idx={best_idx}/{N}, "
