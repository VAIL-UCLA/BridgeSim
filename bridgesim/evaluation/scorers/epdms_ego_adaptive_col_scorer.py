"""
EPDMS Ego Adaptive Collision Scorer — Plan continuity + collision scoring.

Blends EPDMSEgoScorerV1 (adaptive trajectory selection with plan continuity)
and EPDMSEgoTTCScorer (collision scoring via linear interpolation of agents).

Key features:
1. Caches the previous best trajectory for continuity (from V1).
...
5. Includes COL metric via linear extrapolation of other agents' future positions.
...
class EPDMSEgoAdaptiveColScorer(BaseTrajectoryScorer):
    """
    EPDMS Ego Adaptive Collision Scorer with plan continuity and collision scoring.
    ...
    Includes COL metric via linear extrapolation of other agents.
    """
    ...
    def select_best(self, model_output: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        if not self._initialized:
            raise RuntimeError("EPDMSEgoAdaptiveColScorer not initialized.")
