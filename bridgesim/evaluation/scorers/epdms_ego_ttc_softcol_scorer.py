"""
EPDMS ego trajectory scorer with soft collision penalty via linear interpolation.

Based on epdms_ego_ttc_scorer.py. Uses linear interpolation of other agents'
current state to infer their future positions, enabling collision scoring
without ground-truth futures.
Metrics: COL, DAC, DDC, TLC, EP, LK, HC, EC.

Scoring formula (soft COL — COL is additive, not multiplicative):
  (DAC × DDC × TLC) × (5×COL + 5×EP + 2×LK + 2×HC + 2×EC) / 16
...
W_TOTAL = W_COLLISION + W_PROGRESS + W_LANE_KEEPING + W_HISTORY_COMFORT + W_EXTENDED_COMFORT
...
class EPDMSEgoTTCSoftColScorer(BaseTrajectoryScorer):
    """
    EPDMS scorer with soft collision penalty via linear interpolation of agents.

    Unlike EPDMSEgoTTCScorer where COL is a multiplicative gate that zeros
    out the score on collision, here COL is an additive weighted component.
    This means collisions reduce the score but don't completely eliminate it.

    Formula: (DAC × DDC × TLC) × (5×COL + 5×EP + 2×LK + 2×HC + 2×EC) / 16
    """
    ...
    def initialize(self, scenario_data: dict, env):
        ...
        if sdc_track['state']['valid'][0]:
            ...
            print(f"[EPDMS Ego TTC SoftCol] Calibrated World->Sim Offset: {self.world_to_sim_offset}")
        else:
            self.world_to_sim_offset = np.array([0.0, 0.0])
            print("[EPDMS Ego TTC SoftCol] Could not calibrate coordinates (Frame 0 invalid).")

        self.prev_frame_idx = None
        self._initialized = True
        print(f"[EPDMS Ego TTC SoftCol] Initialized with {len(self.all_lanes)} lanes (NC/TTC via linear interpolation)")
    ...
    def _calculate_metrics(self, states: dict, horizon: int, frame_idx: int,
        ...
        # 2. COL (discounted per-timestep collision score, γ^(t*dt) weighting)
        #    Per-timestep score: 1.0 = no collision, 0.5 = not-at-fault, 0.0 = at-fault
        if agents_per_t is not None:
            col_num, col_den = 0.0, 0.0
            for t in range(horizon):
                w = self.gamma ** (t * self.planner_dt)
                step_score = 1.0  # no collision
    ...
    def select_best(self, model_output: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        if not self._initialized:
            raise RuntimeError("EPDMSEgoTTCSoftColScorer not initialized.")
        ...
        for i in range(N):
            ...
            weighted_sum = (W_COLLISION * metrics['col'] +
                            W_PROGRESS * metrics['ep'] +
        ...
        print(f"[EPDMS Ego TTC SoftCol] Frame {frame_idx}: best_idx={best_idx}/{N}, "
