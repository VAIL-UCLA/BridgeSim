"""
EPDMS ego trajectory scorer with soft collision and soft DAC penalty.

Based on epdms_ego_ttc_softcol_scorer.py. Uses linear interpolation of other
agents' current state to infer their future positions, enabling collision scoring
without ground-truth futures.
Metrics: COL, DAC, DDC, TLC, EP, LK, HC, EC.

Scoring formula (DAC is multiplicative, COL is additive):
  (DAC × DDC × TLC) × (20×COL + 0×EP + 0×LK + 0×HC + 0×EC) / 20
...
DAC is a discounted per-timestep drivable area compliance score:
  Σ γ_dac^(t·dt) · score_t / Σ γ_dac^(t·dt)
...
W_PROGRESS = 0.0
W_LANE_KEEPING = 0.0
W_HISTORY_COMFORT = 0.0
W_EXTENDED_COMFORT = 0.0
...
class EPDMSEgoTTCSoftColSoftDACScorer(BaseTrajectoryScorer):
    """
    EPDMS scorer with soft collision and soft DAC penalty.

    DAC is a multiplicative gate in the first parenthesis alongside DDC and TLC.
    COL is an additive weighted component in the second parenthesis.

    Formula: (DAC × DDC × TLC) × (20×COL + 0×EP + 0×LK + 0×HC + 0×EC) / 20
    """
    ...
    def initialize(self, scenario_data: dict, env):
        ...
        if sdc_track['state']['valid'][0]:
            ...
            print(f"[EPDMS Ego TTC SoftCol SoftDAC] Calibrated World->Sim Offset: {self.world_to_sim_offset}")
        else:
            self.world_to_sim_offset = np.array([0.0, 0.0])
            print("[EPDMS Ego TTC SoftCol SoftDAC] Could not calibrate coordinates (Frame 0 invalid).")

        self.prev_frame_idx = None
        self._initialized = True
        print(f"[EPDMS Ego TTC SoftCol SoftDAC] Initialized with {len(self.all_lanes)} lanes (NC/TTC via linear interpolation)")
    ...
    def _calculate_metrics(self, states: dict, horizon: int, frame_idx: int,
        ...
        # 1. DAC (discounted per-timestep drivable area compliance, γ^(t·dt) weighting)
    ...
    def select_best(self, model_output: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        if not self._initialized:
            raise RuntimeError("EPDMSEgoTTCSoftColSoftDACScorer not initialized.")
        ...
        print(f"[EPDMS Ego TTC SoftCol SoftDAC] Frame {frame_idx}: best_idx={best_idx}/{N}, "
