"""
EPDMS ego trajectory scorer with collision scoring via linear interpolation.

Based on epdms_trajectory_scorer_fast.py. Uses linear interpolation of
other agents' current state to infer their future positions, enabling
...
class EPDMSEgoTTCScorer(BaseTrajectoryScorer):
    """
    EPDMS scorer with collision scoring via linear interpolation of agents.

    Other agents' future positions are extrapolated linearly from their
    current state. Metrics: COL, DAC, DDC, TLC, EP, LK, HC, EC.
    """
    ...
    def initialize(self, scenario_data: dict, env):
        ...
        if sdc_track['state']['valid'][0]:
            ...
            print(f"[EPDMS Ego TTC] Calibrated World->Sim Offset: {self.world_to_sim_offset}")
        else:
            self.world_to_sim_offset = np.array([0.0, 0.0])
            print("[EPDMS Ego TTC] Could not calibrate coordinates (Frame 0 invalid).")

        self.prev_frame_idx = None
        self._initialized = True
        print(f"[EPDMS Ego TTC] Initialized with {len(self.all_lanes)} lanes (NC/TTC via linear interpolation)")
    ...
    def select_best(self, model_output: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        print(f"[EPDMS Ego TTC] Frame {frame_idx}: best_idx={best_idx}/{N}, "
