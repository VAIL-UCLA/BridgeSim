...
# Weights & Thresholds (no EC weight)
W_PROGRESS, W_TTC, W_LANE_KEEPING, W_HISTORY_COMFORT = 5.0, 5.0, 2.0, 2.0
...
class PDMSScorer:
    """PDMS scorer: EPDMS without Extended Comfort (EC).

    Formula: (NC × DAC × DDC × TLC) × (5×EP + 5×TTC + 2×LK + 2×HC) / 14
    Closed-loop (no EP): (NC × DAC × DDC × TLC) × (5×TTC + 2×LK + 2×HC) / 9
    """
    ...
    def __init__(self, scenario_data: dict, env):
        ...
        if sdc_track['state']['valid'][0]:
            ...
            print(f"[PDMS INIT] Calibrated World->Sim Offset: {self.world_to_sim_offset}")
        else:
            self.world_to_sim_offset = np.array([0.0, 0.0])
            print("[PDMS WARN] Could not calibrate coordinates (Frame 0 invalid).")
    ...
    def score_frame(self, plan_traj: np.ndarray, frame_idx: int) -> Dict[str, float]:
        ...
        keys = ['nc', 'dac', 'ddc', 'tlc', 'ep', 'ttc', 'lk', 'hc']
        for m in keys:
        ...
        weighted_avg = weighted_sum / (W_PROGRESS + W_TTC + W_LANE_KEEPING + W_HISTORY_COMFORT)
    ...
    def score_frame_no_ep(self, plan_traj: np.ndarray, frame_idx: int) -> Dict[str, float]:
        ...
        Formula: (NC × DAC × DDC × TLC) × (5×TTC + 2×LK + 2×HC) / 9
        """
        sdc_track = self.scenario_data['tracks'][self.sdc_id]
        if not sdc_track['state']['valid'][frame_idx]:
             return {"score": 0.0, "valid": False}
        ...
        keys = ['nc', 'dac', 'ddc', 'tlc', 'ttc', 'lk', 'hc']  # No 'ep'
        for m in keys:
            if human_metrics and human_metrics.get(m) == 0.0:
                final_metrics[m] = 1.0
        ...
        weighted_avg = weighted_sum / (W_TTC + W_LANE_KEEPING + W_HISTORY_COMFORT)
        score = multi_prod * weighted_avg
    ...
    def _check_hc_live(self, ego_pos: np.ndarray, ego_velocity: np.ndarray,
                       ego_heading: float) -> float:
        """Check History Comfort (HC) using actual motion."""
        hc = 1.0
        dt = self.scenario_dt
    ...
    def score_frame_live(self, frame_idx: int) -> Dict[str, float]:
        """
        Score current frame using LIVE simulation state.
        ...
        hc = self._check_hc_live(ego_pos, ego_velocity, ego_heading)

        multi_prod = nc * dac * ddc * tlc
        weighted_sum = (W_TTC * ttc + W_LANE_KEEPING * lk + W_HISTORY_COMFORT * hc)
        weighted_avg = weighted_sum / (W_TTC + W_LANE_KEEPING + W_HISTORY_COMFORT)
        score = multi_prod * weighted_avg
